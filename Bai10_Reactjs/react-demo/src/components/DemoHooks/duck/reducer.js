...
export const movieBookingReducer = (state = initialState, action) => {
    switch (action.type) {
        case CHAIR_BOOKING: {
            ...
            // nếu như tìm thấy nó trả về index của phần tử đó trong mảng
            //  Nếu ko tìm thấy trả về -1
            const index = data.findIndex((chair) => chair.soGhe === action.payload.soGhe)
            if (index !== -1) {
                data.splice(index, 1)
            } else {
        ...
        case PAY: {
            const danhSachSoGheDangChon = state.chairBookingList.map((e) => e.soGhe)
